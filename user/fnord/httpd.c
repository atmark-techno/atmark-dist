...
static void sig_child_ignore(int sig)
{
  ...
  /*fprintf(stderr, "Got sigchild, child %d returned status %d\n", child, status);*/
...
int main(int argc,char *argv[],const char *const *envp) {
...
#ifdef SERVER_MODE
  {
	...
	fprintf(stderr, "Warning: Server mode is for test purposes only, do NOT run in a production environment\n\n");
	...
	while (1) {
	  int child;
	  socklen_t salen = sizeof(saremote);
