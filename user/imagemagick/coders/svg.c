...
#endif
  (void) ThrowMagickException(svg_info->exception,GetMagickModule(),
    DelegateWarning,reason,"`%s`",strerror(errno));
  va_end(operands);
}
...
static Image *ReadSVGImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  svg_info.parser=xmlCreatePushParserCtxt(sax_handler,&svg_info,(char *) NULL,0,
    image->filename);
  while ((n=ReadBlob(image,MaxTextExtent,message)) != 0)
  {
    status=xmlParseChunk(svg_info.parser,(char *) message,(int) n,MagickFalse);
    if (status != 0)
      break;
  }
  (void) xmlParseChunk(svg_info.parser,(char *) message,0,MagickTrue);
...
#endif
  entry->encoder=(EncoderHandler *) WriteSVGImage;
  entry->description=AcquireString("Compressed Scalable Vector Graphics");
  if (*version != '\0')
    entry->version=AcquireString(version);
  entry->magick=(MagickHandler *) IsSVG;
  entry->module=AcquireString("SVG");
