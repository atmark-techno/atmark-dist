  ...
  */
  ...
  for (i=0; i < icon_file.count; i++)
  {
    ...
    */
    ...
    image->matte=(MagickBooleanType) (icon_info.bits_per_pixel == 32);
  ...
  if (image->storage_class == PseudoClass)
    {
      ...
      */
      if (AllocateImageColormap(image,1 << icon_info.bits_per_pixel) == MagickFalse)
        ...
        for (y=(long) image->rows-1; y >= 0; y--)
        {
          ...
          for (x=0; x < (long) image->columns; x++)
          {
            ...
            if (image->matte != MagickFalse)
              q->opacity=(QuantumRange-ScaleCharToQuantum(ReadBlobByte(image)));
            q++;
          }
          if (SyncImagePixels(image) == MagickFalse)
            break;
...
ModuleExport void RegisterICONImage(void)
{
  ...
  entry->module=AcquireString("CUR");
...
%
%      MagickBooleanType WriteICONImage(const ImageInfo *image_info,Image *image)
%
%  A description of each parameter follows.
%
%    o image_info: The image info.
  ...
  Image *image)
{
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    if (image->storage_class != DirectClass)
      {
        ...
        */
        ...
        if (icon_info.number_colors < image->colors)
          ...
          {
            icon_info.file_size+=3*(1 << icon_info.bits_per_pixel);
            icon_info.offset_bits+=3*(1 << icon_info.bits_per_pixel);
            icon_info.file_size+=(1 << icon_info.bits_per_pixel);
            icon_info.offset_bits+=(1 << icon_info.bits_per_pixel);
          }
      }
    if (image->storage_class == DirectClass)
      {
    ...
    if (image->matte == MagickFalse)
    ...
    else
      {
        ...
        icon_info.size=108;
        extra_size=68;
        icon_info.file_size+=extra_size;
        icon_info.offset_bits+=extra_size;
      }
    ...
    */
    ...
    for (y=0; y < (long) image->rows; y++)
    {
      ...
      {
        byte>>=1;
        if (image->matte && (p->opacity == OpaqueOpacity))
          byte|=0x80;
        bit++;
        if (bit == 8)
          {
  ...
  (void) WriteBlobLSBShort(image,0);
  (void) WriteBlobLSBShort(image,1);
  (void) WriteBlobLSBShort(image,scene+1);
  scene=0;
  next=image;
  do
  {
