...
#define VFF_TYP_1_BYTE  1
  ...
  */
  ...
  do
  {
    ...
    */
    viff_pixels=(unsigned char *) AcquireMagickMemory(packets);
    if (viff_pixels == (unsigned char *) NULL)
          ...
          for (x=0; x < (long) image->columns; x++)
          {
            ...
            *(q+number_pixels*2)=ScaleQuantumToChar(p->blue);
            if (image->matte != MagickFalse)
              *(q+number_pixels*3)=ScaleQuantumToChar(QuantumRange-p->opacity);
            p++;
            q++;
          }
          if (image->previous == (Image *) NULL)
    ...
    (void) WriteBlob(image,packets,viff_pixels);
    viff_pixels=(unsigned char *) RelinquishMagickMemory(viff_pixels);
    if (GetNextImageInList(image) == (Image *) NULL)
      break;
    image=SyncNextImageInList(image);
