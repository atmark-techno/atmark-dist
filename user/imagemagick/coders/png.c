...
#define TransformColorspace(image,cs) (void) SetImageColorspace(image,cs)
...
static MagickBooleanType CompressColormapTransFirst(Image *image)
{
  ...
  */
  map=(IndexPacket *) AcquireMagickMemory(number_colors*sizeof(IndexPacket));
      ...
      */
      ...
      if (ping_info->color_type == PNG_COLOR_TYPE_PALETTE)
      ...
      else
        {
          ...
          scale=(QuantumRange/((1 << ping_info->bit_depth)-1));
          if (scale < 1)
             scale=1;
          for (i=0; i < (long) image->colors; i++)
          {
      ...
      */
      ...
      for (y=0; y < (long) image->rows; y++)
      {
        ...
        if (depth == 8 && ping_info->color_type == PNG_COLOR_TYPE_GRAY)
          (void) ExportQuantumPixels(image,GrayQuantum,0,png_pixels+row_offset);
        if (ping_info->color_type == PNG_COLOR_TYPE_GRAY ||
            ping_info->color_type == PNG_COLOR_TYPE_GRAY_ALPHA)
          {
            image->depth=8;
            (void) ExportQuantumPixels(image,GrayAlphaQuantum,0,png_pixels+row_offset);
            image->depth=depth;
          }
        else if (depth == 8 && ping_info->color_type == PNG_COLOR_TYPE_RGB)
           (void) ExportQuantumPixels(image,RGBQuantum,0,png_pixels+row_offset);
        else if (ping_info->color_type == PNG_COLOR_TYPE_RGB ||
              ping_info->color_type == PNG_COLOR_TYPE_RGB_ALPHA)
          {
            image->depth=8;
            (void) ExportQuantumPixels(image,RGBAQuantum,0,png_pixels+row_offset);
            image->depth=depth;
          }
        else if (ping_info->color_type == PNG_COLOR_TYPE_PALETTE)
            (void) ExportQuantumPixels(image,IndexQuantum,0,png_pixels+row_offset);
#else /* (QuantumDepth != 8) */
        if (ping_info->color_type == PNG_COLOR_TYPE_GRAY)
          (void) ExportQuantumPixels(image,GrayQuantum,0,png_pixels+row_offset);
        else if (ping_info->color_type == PNG_COLOR_TYPE_GRAY_ALPHA)
      ...
      if (image->previous == (Image *) NULL)
        if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
            (QuantumTick(pass,num_passes) != MagickFalse))
          {
            status=image->progress_monitor(LoadImageTag,pass,num_passes,
              image->client_data);
  ...
  if (GetPixels(image) != (PixelPacket *) NULL)
    {
      ...
      */
      if (logging != MagickFalse)
        LogMagickEvent(CoderEvent,GetMagickModule(),"  AllocateNextImage()");
...
# endif
      ...
      if (image->depth < QuantumDepth)
        {
          ...
          background.red=(png_uint_16)
            (maxval*image->background_color.red/QuantumRange);
          background.green=(png_uint_16)
            (maxval*image->background_color.green/QuantumRange);
          background.blue=(png_uint_16)
            (maxval*image->background_color.blue/QuantumRange);
          background.gray=(png_uint_16)
            (maxval*PixelIntensity(&image->background_color)/QuantumRange);
        }
      else
        {
          ...
          if (number_colors == 0 || number_colors > 256)
            {
              ...
              quantize_info.number_colors= (matte != MagickFalse ? 255 : 256);
          ...
          if ((y == (long) image->rows) && (x == (long) image->columns))
          ...
          else
            {
              ...
              ping_info->trans_values.index=(unsigned char)
                 (ScaleQuantumToChar(QuantumRange-p->opacity));
            }
          if (ping_info->valid & PNG_INFO_tRNS)
            {
              /*
              ...
              */
              for (y=0; y < (long) image->rows; y++)
              {
                ...
                for (x=(long) image->columns-1; x >= 0; x--)
                {
                  if (p->opacity != OpaqueOpacity)
                    {
                      if (RGBColorMatchExact(ping_info->trans_values,*p) == 0)
                      {
    ...
    if (ping_info->bit_depth < 8 && ping_info->color_type ==
        PNG_COLOR_TYPE_GRAY)
      {
         ...
         ping_info->trans_values.gray=(png_uint_16)(maxval*
           ping_info->trans_values.gray/QuantumRange);
      }
    }
  if (logging != MagickFalse)
    (void) LogMagickEvent(CoderEvent,GetMagickModule(),
  ...
  */
  ...
  quality=image->quality == UndefinedCompressionQuality ? 75 : image->quality;
  if (quality > 9)
    {
      int
        level;
      ...
      */
      for (y=0; y < (long) image->rows; y++)
      {
        ...
        if (p == (const PixelPacket *) NULL)
          break;
        if (mng_info->IsPalette)
              (void) ImportQuantumPixels(image,GrayQuantum,0,png_pixels);
        else
          (void) ImportQuantumPixels(image,RedQuantum,0,png_pixels);
         ...
         (mng_info->optimize || mng_info->IsPalette) && ImageIsGray(image))
      {
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          if (ping_info->color_type == PNG_COLOR_TYPE_GRAY)
            ...
            {
              (void) ImportQuantumPixels(image,GrayAlphaQuantum,0,png_pixels);
            }
          png_write_row(ping,png_pixels);
        }
        if (image->previous == (Image *) NULL)
        ...
        if ((image->depth > 8) || (mng_info->write_png24 || mng_info->write_png32 ||
          ...
          for (y=0; y < (long) image->rows; y++)
          {
            ...
            if (ping_info->color_type == PNG_COLOR_TYPE_GRAY)
              ...
              (void) ImportQuantumPixels(image,RGBAQuantum,0,png_pixels);
            else
              (void) ImportQuantumPixels(image,RGBQuantum,0,png_pixels);
            png_write_row(ping,png_pixels);
            if (logging)
              (void) LogMagickEvent(CoderEvent,GetMagickModule(),
      ...
      else
        {
          for (y=0; y < (long) image->rows; y++)
          {
            ...
            if (ping_info->color_type == PNG_COLOR_TYPE_GRAY)
              ...
              (void) ImportQuantumPixels(image,IndexQuantum,0,png_pixels);
            png_write_row(ping,png_pixels);
          }
        }
        if (image->previous == (Image *) NULL)
   ...
   const ImageInfo *image_info,Image *image)
{
  ...
  jng_quality=image_info->quality == 0 ? 75 : image_info->quality;
  jng_alpha_compression_method=0;
  ...
  */
  ...
  if (transparent)
  {
    ...
    *(chunk+8)=0;
    *(chunk+9)=blue;
    (void) WriteBlob(image,num_bytes,chunk);
  ...
  if (transparent)
    {
      if (jng_alpha_compression_method==0)
      ...
      else
        {
          ...
          (void) WriteBlobMSBULong(image,crc32(crc32(0,chunk,4),blob,length));
        }
     ...
     if (option != (const char *) NULL)
       {
         ...
         */
         ...
         LogPNGChunk(logging,mng_nEED,length);
         length+=4;
         (void) WriteBlob(image,length,chunk);
         (void) WriteBlobMSBULong(image,crc32(0,chunk,length));
       }
