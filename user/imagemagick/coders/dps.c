...
static Image *ReadDPSImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  if ((image->x_resolution != 0.0) && (image->y_resolution != 0.0))
    pixels_per_point=Min(image->x_resolution,image->y_resolution)/
      DefaultResolution;
  ...
  */
  status=XDPSImageFileIntoDrawable((DPSContext) NULL,screen,pixmap,
    ...
    -page.x,-page.y,pixels_per_point,MagickTrue,MagickFalse,MagickTrue,&sans);
  ...
  switch (image->storage_class)
  {
    ...
    case PseudoClass:
    {
      /*
        Create colormap.
      */
      if (AllocateImageColormap(image,visual_info->colormap_size) == MagickFalse)
        {
          image=DestroyImage(image);
