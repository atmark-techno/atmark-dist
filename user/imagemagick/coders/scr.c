  ...
  for(zonenr=0;zonenr<3;zonenr++)
  {
        ...
        for(octetline=0;octetline<8;octetline++)
      {
            ...
            for(z=7;z>-1;z--)
          {
              ...
              if(binar[z])
            {
                q->red=QuantumRange*(colour_palette[3*ink]);
                q->green=QuantumRange*(colour_palette[1+(3*ink)]);
                q->blue=QuantumRange*(colour_palette[2+(3*ink)]);
            } else {
                q->red=QuantumRange*(colour_palette[3*paper]);
                q->green=QuantumRange*(colour_palette[1+(3*paper)]);
                q->blue=QuantumRange*(colour_palette[2+(3*paper)]);
            }
...
%
*/
ModuleExport void RegisterSCRImage(void)
{
  MagickInfo
    *entry;

  entry=SetMagickInfo("SCR");
  entry->decoder=(DecoderHandler *) ReadSCRImage;
  entry->adjoin=MagickFalse;
  entry->description=AcquireString("ZX-Spectrum SCREEN$");
  entry->module=AcquireString("SCR");
