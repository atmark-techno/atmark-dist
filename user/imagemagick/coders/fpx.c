    ...
    if ((y % tile_height) == 0)
      {
        ...
        */
        ...
        if (fpx_status == FPX_LOW_MEMORY_ERROR)
          {
            scanline=(char *) RelinquishMagickMemory(scanline);
            (void) FPX_CloseImage(flashpix);
            FPX_ClearSystem();
            ThrowReaderException(ResourceLimitError,"MemoryAllocationFailed");
          }
...
static MagickBooleanType WriteFPXImage(const ImageInfo *image_info,Image *image)
{
  ...
  if (comment != (ImageAttribute *) NULL)
    {
      ...
      */
      ...
      summary_info.comments.ptr=(unsigned char *)
        AcquireMagickMemory(strlen(comment->value)+MaxTextExtent);
      if (summary_info.comments.ptr == (unsigned char *) NULL)
        ThrowWriterException(DelegateError,"UnableToSetImageComment");
  ...
  */
  pixels=(unsigned char *)
    AcquireMagickMemory(colorspace.numberOfComponents*image->columns);
