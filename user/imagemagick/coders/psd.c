          ...
          switch (channel)
          {
            ...
            case 3:
            {
              if (image->colorspace == CMYKColorspace)
                *indexes=(Quantum) (QuantumRange-pixel);
              else
                q->opacity=(Quantum) (QuantumRange-pixel);
              break;
            }
...
#define MaxPSDChannels  24
  ...
  */
  ...
  /*
    Initialize image.
  */
  image->depth=psd_info.depth <= 8 ? 8UL : QuantumDepth;
  ...
  */
  ...
  if ((image_info->number_scenes == 1) && (image->scene == 0))
    {
      /*
        Skip to the precombined layed.
      */
  ...
  else
    {
      if (length == 0)
      ...
      else
        {
          ...
          (void) ResetMagickMemory(layer_info,0,(size_t)
            number_layers*sizeof(*layer_info));
          for (i=0; i < number_layers; i++)
          {
            ...
            layer_info[i].opacity=(Quantum)
              (QuantumRange-ScaleCharToQuantum(ReadBlobByte(image)));
      ...
      if (image->debug != MagickFalse)
          ...
          combinedlength += 12 + length;  /* sig, key, length + the actual length*/
            ...
            */
            ...
            FormatMagickString(s,MaxTextExtent,"%lu",
        ...
        for (i=0; i < number_layers; i++)
        {
          if (image->debug != MagickFalse)
            ...
            if (layer_info[i].opacity != OpaqueOpacity)
              {
                ...
                */
                for (y=0; y < (long) layer_info[i].image->rows; y++)
                {
                  ...
                  for (x=0; x < (long) layer_info[i].image->columns; x++)
                  {
                    q->opacity=QuantumRange-(Quantum) ((unsigned long)
                      (q->opacity*layer_info[i].opacity)/QuantumRange);
                    q++;
                  }
                  if (SyncImagePixels(layer_info[i].image) == MagickFalse)
                    break;
            ...
            if (layer_info[i].image->colorspace == CMYKColorspace)
              (void) NegateImageChannel(layer_info[i].image,AllChannels,
                MagickFalse);
          }
           ...
           layer for group layer */
       {
         ...
         for (i=number_layers-1; i >=0; i--)
         {
          ...
          if (inside_layer == 0)
           ...
           else if ((inside_layer == 1) && (layer_visible == 0))
             {
       ...
       for (i=0; i < number_layers; i++)
       {
        ...
        if ((image->columns != layer_info[0].image->columns) ||
            (image->rows != layer_info[0].image->rows))
          {
  ...
  */
  ...
  if (image->colorspace == CMYKColorspace)
    (void) NegateImageChannel(image,AllChannels,MagickFalse);
  CloseBlob(image);
...
static void WriteOneChannel(Image *image,Image *tmp_image,unsigned char *pixels,
  QuantumType whichQuantum)
{
  int
    y;
  ...
  for (y=0; y < (long) tmp_image->rows; y++)
  {
    ...
    break;
    (void) ImportQuantumPixels(tmp_image,whichQuantum,0,pixels);
    (void) WriteBlob(image,packet_size*tmp_image->columns,pixels);
  }
}

static void WriteImageChannels(Image* image,Image *tmp_image,
  ...
  if (tmp_image->storage_class == PseudoClass)
  ...
  else
    {
      if (tmp_image->colorspace == CMYKColorspace)
        NegateImageChannel(image,AllChannels,MagickFalse);
      if (tmp_image->matte)
    ...
    while ( tmp_image != NULL ) {
      ...
      if (theAttr) {
        ...
        */
        ...
        FormatMagickString((char *) layer_name,MaxTextExtent,"L%02ld",
          layer_count++ );
