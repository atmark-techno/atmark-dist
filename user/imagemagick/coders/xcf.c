  ...
  */
  for ( i=0; i < (long) nmemb_read_successfully;i++ ) {
    if ( inDocInfo->image_type == GIMP_GRAY ) {
      ...
      q->opacity  = (Quantum) ((int) xcfdata->opacity==0 ? TransparentOpacity : ScaleCharToQuantum(255-inLayerInfo->opacity));
          ...
          if (length == 128)
            ...
            while (length-- > 0)
            {
              ...
              switch (i) {
                ...
                case 3:
                  q->opacity = (Quantum) (ScaleCharToQuantum(255-data));
                break;
              }
              q++;
            }
  ...
  XCFLayerInfo *outLayer )
{
  ...
  outLayer->image->background_color.opacity=
    ScaleCharToQuantum(255-outLayer->opacity);    
