...
ModuleExport void RegisterPS3Image(void)
{
  ...
  entry->description=AcquireString("Level III Encapsulated PostScript");
  entry->module=AcquireString("PS3");
  (void) RegisterMagickInfo(entry);
  entry=SetMagickInfo("PS3");
  entry->encoder=(EncoderHandler *) WritePS3Image;
  entry->description=AcquireString("Level III PostScript");
  entry->module=AcquireString("PS3");
  (void) RegisterMagickInfo(entry);
}

  ...
  do
  {
    ...
    */
    ...
    /*
      Image mask.
    */
    if ((LocaleCompare(image_info->magick,"EPS3") != 0) && image->matte)
      if (WritePS3MaskImage(image_info,image) == MagickFalse)
        {
          CloseBlob(image);
          return(MagickFalse);
        }
    ...
    */
    (void) WriteBlobString(image,
      (LocaleCompare(image_info->magick,"EPS3") != 0) && image->matte ?
      "true\n" : "false\n");
    /*
        ...
        if (compression == JPEGCompression)
        ...
        else
          {
            ...
            switch (compression)
            {
              ...
              {
                for (i=0; i < (long) length; i++)
                  (void) WriteBlobByte(image,pixels[i]);
                break;
              }
              case LZWCompression:
              {
              ...
              case ZipCompression:
              {
                status=ZLIBEncodeImage(image,length,image->quality,pixels);
                break;
              }
            }
            pixels=(unsigned char *) RelinquishMagickMemory(pixels);
