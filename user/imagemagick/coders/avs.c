    ...
    for (y=0; y < (long) image->rows; y++)
    {
      ...
      for (x=0; x < (long) image->columns; x++)
      {
        q->opacity=(Quantum) (QuantumRange-
          ScaleCharToQuantum((unsigned long) *p++));
        q->red=ScaleCharToQuantum((unsigned long) *p++);
        q->green=ScaleCharToQuantum((unsigned long) *p++);
        q->blue=ScaleCharToQuantum((unsigned long) *p++);
