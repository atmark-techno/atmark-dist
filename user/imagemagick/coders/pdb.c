  ...
  unsigned char *source,unsigned long literal,unsigned long repeat)
{
  ...
  CopyMagickMemory(destination,source,literal);
  destination+=literal;
  if (repeat > 0)
    {
...
static MagickBooleanType WritePDBImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  CopyMagickMemory(pdb_info.type,"vIMG",4);
  CopyMagickMemory(pdb_info.id,"View",4);
  pdb_info.seed=0;
              ...
              if (0x7f < literal)
                {
                  ...
                  CopyMagickMemory(buffer,buffer+literal+repeat,0x80);
                  literal-=0x80;
                }
            }
