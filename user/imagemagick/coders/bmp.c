  ...
  */
  ...
  do
  {
    ...
    */
    ...
    image->depth=8;
    image->matte=(MagickBooleanType) (bmp_info.alpha_mask != 0);
        ...
        for (y=(long) image->rows-1; y >= 0; y--)
        {
          ...
          if (image->previous == (Image *) NULL)
            if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
              ...
              {
                status=image->progress_monitor(LoadImageTag,(MagickOffsetType)
                  image->rows-y-1,image->rows,image->client_data);
                if (status == MagickFalse)
                  break;
              }
        }
    ...
    if (image->storage_class == DirectClass)
      {
        ...
        */
        ...
        bmp_info.compression=(MagickBooleanType)
          ((type > 3) && (image->matte != MagickFalse) ? BI_BITFIELDS : BI_RGB);
      }
    bytes_per_line=4*((image->columns*bmp_info.bits_per_pixel+31)/32);
    bmp_info.ba_offset=0;
    profile=GetImageProfile(image,"icc");
    have_color_info=(MagickBooleanType)
