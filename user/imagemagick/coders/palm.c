  ...
  */
  ...
  do
  {
    ...
    */
    ...
    if (bits_per_pixel == 16)  /* Direct Color */
      {
        ...
        ReadBlobByte(image);  /* reserved by Palm */
        transpix.red=QuantumRange*ReadBlobByte(image)/31;
        transpix.green=QuantumRange*ReadBlobByte(image)/63;
        transpix.blue=QuantumRange*ReadBlobByte(image)/31;
      }
    if (bits_per_pixel == 8)
      {
    ...
    /* TODO move to point just before first use */
      ...
      else
        {
          ...
          if (compressionType == PALM_COMPRESSION_SCANLINE)
            {  
              ...
              CopyMagickMemory(lastrow, one_row, bytes_per_row);
            }
        }
      ptr=one_row;
      ...
      if (bits_per_pixel == 16)
        {
          for (x=0; x < (long) image->columns; x++)
          {
            color16=(*ptr++ << 8);
            color16|=(*ptr++);
            q->red=(QuantumRange*((color16 >> 11) & 0x1f))/0x1f;
            q->green=(QuantumRange*((color16 >> 5) & 0x3f))/0x3f;
            q->blue=(QuantumRange*((color16 >> 0) & 0x1f))/0x1f;
    ...
    if (flags & PALM_HAS_TRANSPARENCY_FLAG)
      {
        ...
        PaintTransparentImage(image,&transpix,TransparentOpacity);
      }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  RegisterPALMImage() adds attributes for the PALM image format to the list of
...
%
*/
static MagickBooleanType WritePALMImage(const ImageInfo *image_info,Image *image)
{
  int
    y;
  ...
  */
  ...
  do
  {
    ...
    if (bits_per_pixel < 16)
      SetImageColorspace(image,image->colorspace);
    if (bits_per_pixel < 8)
      {
    ...
    (void) WriteBlobByte(image,transparentIndex);
    if (image->compression == RLECompression)
    ...
    if (bits_per_pixel == 8)
      {
      ...
      else  /* Map colors to Palm standard colormap */
        {
          ...
          SetImageColorspace(map,map->colorspace);
    ...
    if (flags & PALM_IS_COMPRESSED_FLAG)
      WriteBlobMSBShort(image,0);  /* fill in size later */
    lastrow=(unsigned char *) NULL;
    if (image->compression == FaxCompression)
      lastrow=(unsigned char *) AcquireMagickMemory(bytes_per_row);
      ...
      if (bits_per_pixel == 16)
        {
          ...
          {
            color16=(((31*p->red)/QuantumRange) << 11) | (((63*p->green)/QuantumRange) << 5) |
    ...
    if (flags & PALM_HAS_TRANSPARENCY_FLAG)
      {
        SeekBlob(image,currentOffset+6,SEEK_SET);
        (void) WriteBlobMSBShort(image,flags);
        SeekBlob(image,currentOffset+12,SEEK_SET);
        (void) WriteBlobByte(image,transparentIndex);  /* trans index */
      }
    if (bits_per_pixel == 16)
      {
        SeekBlob(image,currentOffset+20,SEEK_SET);
        WriteBlobByte(image,0);  /* reserved by Palm */
        WriteBlobByte(image,(31*transpix.red)/QuantumRange);
        WriteBlobByte(image,(63*transpix.green)/QuantumRange);
        WriteBlobByte(image,(31*transpix.blue)/QuantumRange);
      }
    if (flags & PALM_IS_COMPRESSED_FLAG)  /* fill in size now */
      {
        SeekBlob(image,currentOffset+offset,SEEK_SET);
        (void) WriteBlobMSBShort(image,GetBlobSize(image)-currentOffset-offset);
      }
    if (one_row != (unsigned char *) NULL) 
      one_row=(unsigned char *) RelinquishMagickMemory(one_row);
    if (lastrow != (unsigned char *) NULL) 
    ...
    SeekBlob(image,currentOffset+10,SEEK_SET);
    nextDepthOffset=(unsigned long) ((GetBlobSize(image)-currentOffset)/4);
    (void) WriteBlobMSBShort(image,nextDepthOffset);
    currentOffset=(MagickOffsetType) GetBlobSize(image);
    SeekBlob(image,currentOffset,SEEK_SET);
    image=SyncNextImageInList(image);
