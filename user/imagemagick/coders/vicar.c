...
%
%      MagickBooleanType IsVICAR(const unsigned char *magick,const size_t length)
%
...
%
*/
static MagickBooleanType IsVICAR(const unsigned char *magick,const size_t length)
{
  if (length < 7)
    return(MagickFalse);
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    (void) ExportQuantumPixels(image,GrayQuantum,0,scanline);
