    ...
    if (IsSync != 0)
      {
        ...
        */
        ...
        if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
            (QuantumTick(row,image->rows) != MagickFalse))
          {
            status=image->progress_monitor(DecodeImageTag,row,image->rows,
              image->client_data);
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  commands[0]=GetClientName();
  commands[1]="-label";
  commands[2]=DefaultTileLabel;
  (void) MogrifyImages(mogrify_info,3,commands,&image,exception);
