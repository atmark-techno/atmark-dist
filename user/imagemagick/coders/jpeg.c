...
static boolean ReadIPTCProfile(j_decompress_ptr jpeg_info)
{
  ...
  *tag='\0';
  ...
  error_manager=(ErrorManager *) jpeg_info->client_data;
  image=error_manager->image;
  iptc_profile=GetImageProfile(image,"iptc");
  ...
  ExceptionInfo *exception)
{
    ...
    for (i=0; i < NUM_QUANT_TBLS; i++)
    {
     ...
     if ((jpeg_info.quant_tbl_ptrs[0] != NULL) &&
         (jpeg_info.quant_tbl_ptrs[1] != NULL))
       {
         ...
         hashval=(int) (jpeg_info.quant_tbl_ptrs[0]->quantval[2]+
  ...
  */
  ...
  if (jpeg_info.quantize_colors != MagickFalse)
    {
      image->colors=jpeg_info.actual_number_of_colors;
    ...
    if (jpeg_info.data_precision > 8)
      {
        if (jpeg_info.output_components == 1)
          ...
          for (x=0; x < (long) image->columns; x++)
          {
            q->red=ScaleShortToQuantum(GETJSAMPLE(*p++) << 4);
            q->green=ScaleShortToQuantum(GETJSAMPLE(*p++) << 4);
            q->blue=ScaleShortToQuantum(GETJSAMPLE(*p++) << 4);
            if (image->colorspace == CMYKColorspace)
              indexes[x]=ScaleShortToQuantum(GETJSAMPLE(*p++) << 4);
            q++;
          }
      }
    else
      if (jpeg_info.output_components == 1)
        ...
        for (x=0; x < (long) image->columns; x++)
        {
          ...
          if (image->colorspace == CMYKColorspace)
            indexes[x]=ScaleCharToQuantum(GETJSAMPLE(*p++));
          q++;
        }
...
ModuleExport void RegisterJPEGImage(void)
{
  ...
  entry->adjoin=MagickFalse;
  entry->description=
    AcquireString("Progessive Joint Photographic Experts Group JFIF");
  entry->module=AcquireString("JPEG");
  (void) RegisterMagickInfo(entry);
}

    ...
    if (LocaleCompare(name,"IPTC") == 0)
      {
        ...
        tag_length=14;
        (void) CopyMagickMemory(custom_profile->datum,"Photoshop 3.0 ",
          tag_length);
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          for (x=0; x < (long) image->columns; x++)
          {
            /*
              Convert DirectClass packets to contiguous CMYK scanlines.
            */
            *q++=(JSAMPLE) (ScaleQuantumToChar(QuantumRange-p->red));
            *q++=(JSAMPLE) (ScaleQuantumToChar(QuantumRange-p->green));
            *q++=(JSAMPLE) (ScaleQuantumToChar(QuantumRange-p->blue));
            *q++=(JSAMPLE) (ScaleQuantumToChar(QuantumRange-indexes[x]));
            p++;
          }
          (void) jpeg_write_scanlines(&jpeg_info,scanline,1);
          if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
