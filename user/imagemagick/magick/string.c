...
MagickExport char *CloneString(char **destination,const char *source)
{
  ...
  *destination=(char *)
    ResizeMagickMemory(*destination,length*sizeof(*destination));
  if (*destination == (char *) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,"UnableToAcquireString",
      source);
  (void) strcpy(*destination,source);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  ConstantString() trims a string of any excess memory.  This method is useful
%  for strings that will not change over the life of the string.
%
%  The format of the ConstantString method is:
%
%      char *ConstantString(char *source)
%
%  A description of each parameter follows:
%
%    o source: A character string.
%
*/
MagickExport char *ConstantString(char *source)
{
  (void) LogMagickEvent(TraceEvent,GetMagickModule(),"...");
  if (source == (char *) NULL)
    return((char *) NULL);
  source=(char *) ResizeMagickMemory(source,(strlen(source)+1)*sizeof(*source));
  if (source == (char *) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,"UnableToAcquireString",
      strerror(errno));
  return(source);
