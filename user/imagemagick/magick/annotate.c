    ...
    if (draw_info->undercolor.opacity != TransparentOpacity)
      {
        ...
        */
        ...
        DestroyDrawInfo(undercolor_info);
      }
  ...
  const DrawInfo *draw_info,TypeMetric *metrics)
{
  ...
  */
  ...
  metrics->height=number_lines*(long) (metrics->ascent-metrics->descent+0.5)+1;
  /*
    Free resources.
  */
    ...
    if (draw_info->render != MagickFalse)
      {
        ...
        */
        ...
        for (y=0; y < (long) bitmap->bitmap.rows; y++)
        {
          ...
          q=GetImagePixels(image,x_offset,y_offset,bitmap->bitmap.width,1);
  ...
  const DrawInfo *draw_info,const PointInfo *offset,TypeMetric *metrics)
{
  ...
  */
  identity=(MagickBooleanType)
    ((draw_info->affine.sx == draw_info->affine.sy) &&
