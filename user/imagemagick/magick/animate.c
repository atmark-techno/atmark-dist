...
MagickExport MagickBooleanType AnimateImageCommand(ImageInfo *image_info,
...
{ \
  ...
  */
  ...
  DestroyAnimate();
  return(status ? MagickTrue : MagickFalse);
#else
  ...
  if (GetNextImageInList(images) != (Image *) NULL)
    {
      ...
      */
      ...
      coalesce=(MagickBooleanType) (next != (Image *) NULL);
        ...
        */
        for (next=images; next != (Image *) NULL; )
        {
          ...
          for (i=0; i < (long) images->colors; i++)
            if (ColorMatch(next->colormap+i,images->colormap+i) == MagickFalse)
              break;
  ...
  for (scene=1; scene < (int) number_scenes; scene++)
  {
    ...
    */
    ...
    if (image_list[scene]->matte)
      (void) XClearWindow(display,window_info.id);
    delay=1000*images->delay/Max(images->ticks_per_second,1);
  ...
  if (GetNextImageInList(images) != (Image *) NULL)
    {
      ...
      */
      ...
      coalesce=next != (Image *) NULL ? MagickTrue : MagickFalse;
      if (coalesce != MagickFalse)
        {
          Image
            *coalesce_image;
        ...
        if (event.xclient.message_type == windows->dnd_protocols)
          {
            ...
            */
            ...
            status=(MagickBooleanType) XGetWindowProperty(display,root_window,
              selection,0L,2047L,MagickFalse,(Atom) AnyPropertyType,&type,
