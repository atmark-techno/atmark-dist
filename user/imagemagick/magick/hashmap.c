...
%
%      void *GetLastValueInLinkedList(const LinkedListInfo *list_info)
%
%  A description of each parameter follows:
%
...
%
*/
MagickExport void *GetLastValueInLinkedList(const LinkedListInfo *list_info)
{
  assert(list_info != (LinkedListInfo *) NULL);
  assert(list_info->signature == MagickSignature);
  if (list_info->debug != MagickFalse)
...
%
%      void *GetValueFromHashmap(const HashmapInfo *hashmap_info,void *key)
%
%  A description of each parameter follows:
%
%    o hashmap_info: The hashmap info.
...
%
*/
MagickExport void *GetValueFromHashmap(const HashmapInfo *hashmap_info,
  const void *key)
{
  LinkedListInfo
    *list_info;
...
%
%      void *GetValueFromLinkedList(const LinkedListInfo *list_info,
...
%
*/
MagickExport void *GetValueFromLinkedList(const LinkedListInfo *list_info,
  const unsigned long index)
{
  register ElementInfo
    *next;
            ...
            *replace=next->next->value;
            ...
            if (element != (ElementInfo *) NULL)
              element->next=(ElementInfo *)
                RelinquishMagickMemory(element->next);
            list_info->elements--;
          }
        if (element != (ElementInfo *) NULL)
          element->next=next;
...
MagickExport MagickBooleanType IsHashmapEmpty(const HashmapInfo *hashmap_info)
{
  ...
  if (hashmap_info->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"...");
  return((MagickBooleanType) (hashmap_info->entries == 0));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  const LinkedListInfo *list_info)
{
  ...
  if (list_info->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"...");
  return((MagickBooleanType) (list_info->elements == 0));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  void *(*relinquish_key)(void *),void *(*relinquish_value)(void *))
{
  ...
  hashmap_info->map=(LinkedListInfo **)
    AcquireMagickMemory((size_t) (capacity+1)*sizeof(*hashmap_info->map));
