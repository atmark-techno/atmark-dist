  ...
  unsigned long *number_aliases,ExceptionInfo *exception)
{
  ...
  *number_aliases=0;
  ...
  for (i=0; p != (const MagicInfo *) NULL; )
  {
    if ((p->stealth == MagickFalse) &&
        (GlobExpression(p->name,pattern) != MagickFalse))
      aliases[i++]=ConstantString(AcquireString(p->name));
    p=(const MagicInfo *) GetNextValueInLinkedList(magic_list);
  }
  RelinquishSemaphoreInfo(magic_semaphore);
    ...
    if (LocaleCompare(keyword,"<magic") == 0)
      {
        ...
        */
        ...
        magic_info->path=ConstantString(AcquireString(filename));
    ...
    switch (*keyword)
    {
      ...
      case 'n':
      {
        if (LocaleCompare((char *) keyword,"name") == 0)
          {
            magic_info->name=ConstantString(AcquireString(token));
            break;
          }
        break;
      }
      ...
      case 's':
      {
        if (LocaleCompare((char *) keyword,"stealth") == 0)
          {
            magic_info->stealth=(MagickBooleanType)
        ...
        if (LocaleCompare((char *) keyword,"target") == 0)
          {
            ...
            magic_info->target=AcquireString(token);
            magic_info->magic=(unsigned char *) AcquireString(token);
            q=magic_info->magic;
