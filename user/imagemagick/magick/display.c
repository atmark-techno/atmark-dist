...
MagickExport MagickBooleanType DisplayImageCommand(ImageInfo *image_info,
...
{ \
  ...
  */
  ...
  /*
    Parse command line.
  */
  for (i=1; ((i <= argc) && ((state & ExitState) == 0)); i++)
  {
    if (i < (long) argc)
      option=argv[i];
    else
        ...
        */
        ...
        for (scene=first_scene; scene <= last_scene ; scene++)
        {
          ...
          */
          ...
          if ((state & FormerImageState) == 0)
            {
              last_image=(MagickBooleanType) image_number;
    ...
    switch (*(option+1))
    {
      ...
      case 'b':
      {
        if (LocaleCompare("backdrop",option+1) == 0)
          {
            resource_info.backdrop=(MagickBooleanType) (*option == '-');
            break;
          }
      ...
      case 'i':
      {
        ...
        if (LocaleCompare("iconic",option+1) == 0)
          {
            resource_info.iconic=(MagickBooleanType) (*option == '-');
            break;
          }
        if (LocaleCompare("immutable",option+1) == 0)
          {
            resource_info.immutable=(MagickBooleanType) (*option == '-');
            break;
          }
      ...
      case 's':
      {
        ...
        if (LocaleCompare("shared-memory",option+1) == 0)
          {
            resource_info.use_shared_memory=(MagickBooleanType)
              (*option == '-');
            break;
          }
      ...
      case 'u':
      {
        ...
        if (LocaleCompare("use_pixmap",option+1) == 0)
          {
            resource_info.use_pixmap=(MagickBooleanType) (*option == '-');
            break;
          }
        ThrowDisplayException(OptionError,"UnrecognizedOption",option);
      }
  ...
  XResourceInfo *resource_info,XWindows *windows,Image *image)
{
        ...
        switch (AnnotateCommands[id])
        {
          ...
          case AnnotateFontColorCommand:
          {
            ...
            */
            ...
            if (pen_number < 0)
              break;
            transparent_pen=(MagickBooleanType)
              (pen_number == (MaxNumberPens-2));
          ...
          case AnnotateBackgroundColorCommand:
          {
            ...
            */
            ...
            if (pen_number < 0)
              break;
            transparent_box=(MagickBooleanType)
              (pen_number == (MaxNumberPens-2));
  ...
  */
  ...
  while (annotate_info != (XAnnotateInfo *) NULL)
  {
    ...
    */
    status=(int) XAnnotateImage(display,windows->pixel_info,
      annotate_info,image);
    if (status == 0)
      return(MagickFalse);
    /*
...
#define BackgroundImageTag  "Background/Image"
  ...
  */
  status=(MagickBooleanType) XDialogWidget(display,windows,"Background",
  ...
  background_resources.backdrop=(MagickBooleanType) status;
            ...
            if ((*image)->storage_class == DirectClass)
              {
                for (y=0; y < (long) (*image)->rows; y++)
                {
                  ...
                  {
                    if (FuzzyColorCompare(*image,q,&target))
                      {
                        q->red=ScaleShortToQuantum(color.red);
                        q->green=ScaleShortToQuantum(color.green);
                        q->blue=ScaleShortToQuantum(color.blue);
            ...
            else
              {
                for (i=0; i < (int) (*image)->colors; i++)
                  if (FuzzyColorCompare(*image,(*image)->colormap+i,&target))
                    {
          ...
          switch (DrawCommands[id])
          {
            ...
            case DrawColorCommand:
            {
              ...
              */
              ...
              if (pen_number < 0)
                break;
              transparent=(MagickBooleanType) (pen_number == (MaxNumberPens-2));
            ...
            case DrawStippleCommand:
            {
              ...
              */
              ...
              status=(MagickBooleanType) XReadBitmapFile(display,root_window,
                filename,&width,&height,&stipple,&x,&y);
  ...
  switch (command)
  {
    ...
    case DeleteCommand:
    {
      ...
      */
      ...
      if (*filename == '\0')
        break;
      status=(MagickBooleanType) remove(filename);
      if (status != MagickFalse)
    ...
    case NewCommand:
    {
      ...
      */
      status=(MagickBooleanType) XDialogWidget(display,windows,"New",
        "Enter image geometry:",geometry);
      if (*geometry == '\0')
        break;
      if (status == MagickFalse)
        format=(char *) "xc";
    ...
    case ResizeCommand:
    {
      ...
      */
      ...
      (void) FormatMagickString(geometry,MaxTextExtent,"%lux%lu+0+0",
        width,height);
      status=(MagickBooleanType) XDialogWidget(display,windows,"Resize",
    ...
    case QuantizeCommand:
    {
      ...
      */
      status=(MagickBooleanType) XDialogWidget(display,windows,"Quantize",
      ...
      */
      ...
      argv[1]="-colors";
      argv[2]=colors;
      argv[3]=(char *) (status != 0 ? "-dither" : "+dither");
    ...
    case ShadeCommand:
    {
      ...
      */
      status=(MagickBooleanType) XDialogWidget(display,windows,"Shade",
      ...
      */
      XSetCursorState(display,windows,MagickTrue);
      XCheckRefreshWindows(display,windows);
      argv[1]=(char *) (status != 0 ? "+shade" : "-shade");
      argv[2]=geometry;
      (void) MogrifyImage(resource_info->image_info,3,argv,image,
        &(*image)->exception);
      XSetCursorState(display,windows,MagickFalse);
  ...
  XWindows *windows,Image *image)
{
  ...
  */
  ...
  status=XMakeImage(display,resource_info,&windows->pan,image,
    windows->pan.width,windows->pan.height);
  if (status == MagickFalse)
    ThrowXWindowException(XServerError,image->exception.reason,
      strerror(errno));
  ...
  XResourceInfo *resource_info,XWindows *windows,Image *image)
{
  ...
  */
  if (resource_info->copy_image == (Image *) NULL)
    return(MagickFalse);
  paste_image=CloneImage(resource_info->copy_image,0,0,MagickTrue,&image->exception);
  /*
    Map Command widget.
  */
    ...
    switch (event.type)
    {
      ...
      case KeyPress:
      {
        ...
        *(command+length)='\0';
        if (image->debug != MagickFalse)
          (void) LogMagickEvent(X11Event,GetMagickModule(),
            "Key press: 0x%lx (%s)",key_symbol,command);
        switch ((int) key_symbol)
        {
          case XK_Escape:
          case XK_F20:
  ...
  if ((LocaleCompare(image_info->magick,"JPEG") == 0) ||
      (LocaleCompare(image_info->magick,"JPG") == 0))
    {
      ...
      */
      ...
      status=(MagickBooleanType) XDialogWidget(display,windows,"Save",
        "Enter JPEG quality:",quality);
      if (*quality == '\0')
        return(MagickTrue);
...
#define XClientName  "montage"
  ...
  */
  backdrop=(windows->visual_info->class == TrueColor) ||
    (windows->visual_info->class == DirectColor) ? MagickTrue :
    MagickFalse;
  ...
  */
  ...
  montage_image=MontageImages(GetFirstImageInList(images),montage_info,
    &images->exception);
...
%
*/
static MagickBooleanType XDisplayBackgroundImage(Display *display,
  XResourceInfo *resource_info,Image *image)
{
  char
    geometry[MaxTextExtent],
  ...
  */
  ...
  status=(MagickBooleanType)
    XGetWindowAttributes(display,window_info.id,&window_attributes);
  if (status != MagickFalse)
  ...
  */
  ...
  XDelay(display,delay == 0 ? 10 : delay);
  (void) XSync(display,MagickFalse);
...
#define MagickTitle  "Commands"
  ...
  */
  ...
  if ((resource_info->title != (char *) NULL) && !(*state & MontageImageState))
    {
      (void) CloneString(&windows->image.name,TranslateText(
        resource_info->image_info,display_image,resource_info->title));
      (void) CloneString(&windows->image.icon_name,TranslateText(
        resource_info->image_info,display_image,resource_info->title));
    }
  else
    {
  ...
  do
  {
    ...
    */
    ...
    if (windows->image.stasis == MagickFalse)
      windows->image.stasis=(MagickBooleanType)
        ((time((time_t *) NULL)-timestamp) > 0);
    if (windows->magnify.stasis == MagickFalse)
      windows->magnify.stasis=(MagickBooleanType)
        ((time((time_t *) NULL)-timestamp) > 0);
    if (event.xany.window == windows->command.id)
      {
        /*
    ...
    switch (event.type)
    {
        ...
        if (event.xclient.message_type == windows->dnd_protocols)
          {
            ...
            */
            ...
            status=(MagickBooleanType) XGetWindowProperty(display,root_window,
              selection,0L,(long) MaxTextExtent,MagickFalse,(Atom)
      ...
      case PropertyNotify:
      {
        ...
        */
        status=(MagickBooleanType) XGetWindowProperty(display,
          event.xproperty.window,event.xproperty.atom,0L,(long) MaxTextExtent,
