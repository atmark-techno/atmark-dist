...
MagickExport Image *CoalesceImages(Image *image,ExceptionInfo *exception)
{
  ...
  */
  ...
  (void) CompositeImage(coalesce_image,image->matte != MagickFalse ?
    OverCompositeOp : CopyCompositeOp,image,image->page.x,image->page.y);
  ...
  for ( ; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    switch (next->dispose)
    {
      ...
      case NoneDispose:
      {
        coalesce_image->next=CloneImage(coalesce_image,0,0,MagickTrue,
          exception);
        if (GetNextImageInList(coalesce_image) != (Image *) NULL)
          previous_image=GetNextImageInList(coalesce_image);
        break;
      }
      case BackgroundDispose:
      {
        ...
        if (GetNextImageInList(coalesce_image) != (Image *) NULL)
          {
            coalesce_image->next->background_color.opacity=TransparentOpacity;
            SetImageBackgroundColor(GetNextImageInList(coalesce_image));
          }
        break;
      }
      ...
      default:
      {
        coalesce_image->next=CloneImage(previous_image,0,0,MagickTrue,
          exception);
        break;
      }
    }
    ...
    coalesce_image->ticks_per_second=next->ticks_per_second;
  ...
  */
  ...
  for (i=0; i < (long) GetImageListLength(images); i+=4)
  {
    ...
    if (cmyk_image == (Image *) NULL)
      break;
    cmyk_image->storage_class=DirectClass;
  ...
  for (y=0; y < (long) crop_image->rows; y++)
  {
    ...
    if ((indexes != (IndexPacket *) NULL) &&
      ...
      (void) CopyMagickMemory(crop_indexes,indexes,(size_t) crop_image->columns*
        sizeof(*crop_indexes));
    if (SyncImagePixels(crop_image) == MagickFalse)
      break;
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
  ...
  ExceptionInfo *exception)
{
  ...
  */
  bounds=(RectangleInfo *) AcquireMagickMemory((size_t)
    GetImageListLength(image)*sizeof(*bounds));
  ...
  for ( ; next != (const Image *) NULL; next=GetNextImageInList(next))
  {
    ...
    */
    for (x=0; x < (long) next->columns; x++)
    {
      ...
      q=AcquireImagePixels(next->previous,x,0,1,next->previous->rows,exception);
      if ((p == (const PixelPacket *) NULL) ||
          (q == (const PixelPacket *) NULL))
        break;
      indexes=GetIndexes(next);
      next_indexes=GetIndexes(next->previous);
      for (y=0; y < (long) next->rows; y++)
      {
        SetMagickPixelPacket(p,indexes+x,&pixel);
        SetMagickPixelPacket(q,next_indexes+x,&target);
    ...
    for (y=0; y < (long) next->rows; y++)
    {
      ...
      q=AcquireImagePixels(next->previous,0,y,next->previous->columns,1,
        exception);
      if ((p == (const PixelPacket *) NULL) ||
          (q == (const PixelPacket *) NULL))
        break;
    ...
    bounds[i].width=(unsigned long) (x-bounds[i].x+1);
    ...
    bounds[i].height=(unsigned long) (y-bounds[i].y+1);
    i++;
  }
  /*
    Clone first image in sequence.
  */
  deconstruct_image=CloneImage(image,0,0,MagickTrue,exception);
  ...
  */
  ...
  for ( ; next != (const Image *) NULL; next=GetNextImageInList(next))
  {
    crop_image=CloneImage(next,0,0,MagickTrue,exception);
    if (crop_image == (Image *) NULL)
      break;
    ...
    if (crop_next == (Image *) NULL)
      break;
    AppendImageToList(&deconstruct_image,crop_next);
    i++;
  }
  bounds=(RectangleInfo *) RelinquishMagickMemory(bounds);
  if (next != (Image *) NULL)
    {
      deconstruct_image=DestroyImageList(deconstruct_image);
      return((Image *) NULL);
    }
  return(GetFirstImageInList(deconstruct_image));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  FlattenImage() merges a sequence of images.  This is useful for combining
...
#define SpliceImageTag  "Splice/Image"
  ...
  */
  ...
  if (splice_image == (Image *) NULL)
    return((Image *) NULL);
  splice_image->storage_class=DirectClass;
  /*
    Splice image.
  */
  i=0;
