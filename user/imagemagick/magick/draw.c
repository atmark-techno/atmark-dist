    ...
    if (affine->sx < -MagickEpsilon)
      {
        intercept=(-z+(MagickRealType) image->columns)/affine->sx;
        x=(long) (intercept+MagickEpsilon+0.5);
        if ((double) x > inverse_edge.x1)
          inverse_edge.x1=(double) x;
        intercept=(-z/affine->sx);
        x=(long) (intercept-MagickEpsilon+0.5);
        if ((double) x < inverse_edge.x2)
          inverse_edge.x2=(double) x;
      }
    else
...
static AffineMatrix InverseAffineMatrix(const AffineMatrix *affine)
{
  ...
  inverse_affine.sx=(double) (determinant*affine->sy);
  inverse_affine.rx=(double) (determinant*(-affine->rx));
  inverse_affine.ry=(double) (determinant*(-affine->ry));
  inverse_affine.sy=(double) (determinant*affine->sx);
  inverse_affine.tx=(double) ((-affine->tx)*inverse_affine.sx-
    affine->ty*inverse_affine.ry);
  inverse_affine.ty=(double) ((-affine->tx)*inverse_affine.rx-
    affine->ty*inverse_affine.sy);
  ...
  const Image *composite,const AffineMatrix *affine)
{
  ...
  */
  ...
  extent[0].x=0.0;
  extent[0].y=0.0;
  extent[1].x=(double) composite->columns;
  extent[1].y=0.0;
  extent[2].x=(double) composite->columns;
  extent[2].y=(double) composite->rows;
  extent[3].x=0.0;
  extent[3].y=(double) composite->rows;
  for (i=0; i < 4; i++)
  {
    point=extent[i];
  ...
  */
  ...
  if (edge.y1 < 0)
    edge.y1=0.0;
  if (edge.y2 >= (double) image->rows)
    edge.y2=(double) image->rows-1;
  for (y=(long) (edge.y1+0.5); y <= (long) (edge.y2+0.5); y++)
  {
    ...
    if (inverse_edge.x1 < 0)
      inverse_edge.x1=0.0;
    if (inverse_edge.x2 >= (double) image->columns)
      inverse_edge.x2=(double) image->columns-1;
    x=(long) (inverse_edge.x1+0.5);
    q=GetImagePixels(image,x,y,(unsigned long)
      (inverse_edge.x2-inverse_edge.x1+0.5),1);
    if (q == (PixelPacket *) NULL)
      continue;
    for ( ; x <= (long) (inverse_edge.x2+0.5); x++)
    {
  ...
  for (i=1; i < (long) number_vertices; i++)
  {
    ...
    for (total_length=0.0; maximum_length >= (length+total_length); )
    {
      ...
      if ((n & 0x01) != 0)
        {
          dash_polygon[0]=primitive_info[0];
          dash_polygon[0].point.x=(double) (primitive_info[i-1].point.x+
            dx*total_length/maximum_length);
          dash_polygon[0].point.y=(double) (primitive_info[i-1].point.y+
      ...
      else
        {
          ...
          dash_polygon[j].point.x=(double) (primitive_info[i-1].point.x+
            dx*total_length/maximum_length);
          dash_polygon[j].point.y=(double) (primitive_info[i-1].point.y+
            dy*total_length/maximum_length);
...
static inline MagickBooleanType IsPoint(const char *point)
{
  ...
  return((MagickBooleanType) (p != point));
}
  ...
  */
  ...
  for (q=primitive; *q != '\0'; )
  {
    ...
    */
    ...
    if (i >= (long) number_points)
      ThrowMagickFatalException(ResourceLimitError,"UnableToDrawOnImage",
        strerror(errno));
  ...
  *stroke_opacity=0.0;
  ...
  for (j=0; j < (long) polygon_info->number_edges; j++)
  {
    if ((MagickRealType) y <= (p->bounds.y1-mid-0.5))
      break;
    if ((MagickRealType) y > (p->bounds.y2+mid+0.5))
      {
    ...
    if ((MagickRealType) x > (p->bounds.x2+mid+0.5))
      {
        p++;
        continue;
      }
    for (i=(long) Max(p->highwater,1); i < (long) p->number_points; i++)
    {
      if ((MagickRealType) y <= (p->points[i-1].y-mid-0.5))
        break;
      if ((MagickRealType) y > (p->points[i].y+mid+0.5))
        continue;
  ...
  const DrawInfo *draw_info,const PrimitiveInfo *primitive_info)
{
  ...
  */
  ...
  fill=(MagickBooleanType) ((primitive_info->method == FillToBorderMethod) ||
  ...
  bounds.x1-=(mid+1.0);
  bounds.x1=bounds.x1 < 0.0 ? 0.0 : bounds.x1 >= (double) image->columns ?
    (double) image->columns-1 : bounds.x1;
  bounds.y1-=(mid+1.0);
  bounds.y1=bounds.y1 < 0.0 ? 0.0 : bounds.y1 >= (double) image->rows ?
    (double) image->rows-1 : bounds.y1;
  bounds.x2+=(mid+1.0);
  bounds.x2=bounds.x2 < 0.0 ? 0.0 : bounds.x2 >= (double) image->columns ?
    (double) image->columns-1 : bounds.x2;
  bounds.y2+=(mid+1.0);
  bounds.y2=bounds.y2 < 0.0 ? 0.0 : bounds.y2 >= (double) image->rows ?
        ...
        case ReplaceMethod:
        {
          ...
          for (y=0; y < (long) image->rows; y++)
          {
            ...
            for (x=0; x < (long) image->columns; x++)
            {
              ...
              fill_color=GetFillColor(draw_info,x,y);
              MagickCompositeOver(&fill_color,(MagickRealType)
                fill_color.opacity,q,(MagickRealType) q->opacity,q);
              q++;
            }
            if (SyncImagePixels(image) == MagickFalse)
              break;
  ...
  const DrawInfo *draw_info,const PrimitiveInfo *primitive_info)
{
  ...
  */
  ...
  for (p=primitive_info; p->primitive != UndefinedPrimitive; p+=p->coordinates)
  {
    ...
    q=p+p->coordinates-1;
    closed_path=(MagickBooleanType) ((q->point.x == p->point.x) &&
      (q->point.y == p->point.y));
  ...
  const MagickBooleanType large_arc,const MagickBooleanType sweep)
{
  ...
  if ((theta < 0.0) && (sweep != MagickFalse))
    ...
    if ((theta > 0.0) && (sweep == MagickFalse))
      theta-=(MagickRealType) (2.0*MagickPI);
  arc_segments=ceil(fabs((double) (theta/(0.5*MagickPI+MagickEpsilon))));
  p=primitive_info;
