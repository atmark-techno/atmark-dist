...
#define WindowIsActive(window_info,position)  ( \
  ((position.x >= 0) && (position.y >= 0) &&  \
   ...
   (position.y < (long) window_info.height)) ? MagickTrue : MagickFalse)

/*
  Enum declarations.
*/
  ...
  const char *title,const char **selections,char *item)
{
  ...
  */
  ...
  windows->widget.y=submenu_info.active == MagickFalse ? y-((3*title_height) >> 2) : y;
  if (submenu_info.active)
...
#define NumberPreferences  7
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        ...
        preferences_info[4].raised=resource_info->quantize_info->dither ==
          MagickFalse ? MagickTrue : MagickFalse;
        preferences_info[5].raised=resource_info->colormap !=
  ...
  */
  ...
  resource_info->use_pixmap=
    preferences_info[6].raised == MagickFalse ? MagickTrue : MagickFalse;
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        ...
        slider_info.height=
          scroll_info.height-((slider_info.min_y-scroll_info.y+1) << 1)+2;
        visible_lines=(unsigned int) (scroll_info.height-(height >> 3)-3)/
