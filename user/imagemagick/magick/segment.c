  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  pixel.red=(MagickRealType) ScaleCharToQuantum(threshold);
  threshold=(background->green.center+object->green.center)/2.0;
  pixel.green=(MagickRealType) ScaleCharToQuantum(threshold);
  threshold=(background->blue.center+object->blue.center)/2.0;
  pixel.blue=(MagickRealType) ScaleCharToQuantum(threshold);
  /*
    Free resources.
  */
  for (cluster=head; cluster != (Cluster *) NULL; cluster=next_cluster)
  ...
  short *extrema)
{
  ...
  */
  ...
  for (i=0; i < number_nodes; i++)
  {
    ...
    */
    peak=(MagickBooleanType) (zero_crossing[k].crossings[node->right] == -1);
    index=node->left;
