    ...
    while (res != ERROR_NO_MORE_ITEMS)
      {
        ...
        *pos='\0'; /* Remove (TrueType) from string */
        ...
        type_info->path=ConstantString(AcquireString("Windows Fonts"));
        ...
        for(pos = buffer; *pos != 0 ; pos++)
          if (*pos == ' ')
            *pos = '-';
        type_info->name=ConstantString(AcquireString(buffer));

        /* Fullname */
        type_info->description=ConstantString(AcquireString(value_name));

        /* Format */
        type_info->format=ConstantString(AcquireString("truetype"));
        ...
        LocaleLower(buffer);
        type_info->glyphs=ConstantString(AcquireString(buffer));
        ...
        if ( (LocaleCompare(type_info->name, "Symbol") == 0 ) ||
          ...
          type_info->encoding=ConstantString(AcquireString("AppleRoman"));
        ...
        type_info->family=ConstantString(AcquireString(buffer));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  ImageToHBITMAP() creates a Windows HBITMAP from an IM Image
%
...
MagickExport void *ImageToHBITMAP(Image* image)
{
  ...
  memSize = bitmap.bmWidthBytes * bitmap.bmHeight;
...
#if QuantumDepth == 8
      /* Form of PixelPacket is identical to RGBQUAD when QuantumDepth==8 */
      CopyMagickMemory((void*)pDestPixel,(const void*)pPixels,
