...
#define XBlueGamma(color) ((Quantum) (blue_gamma == 1.0 ? (double) (color) : \
  ((pow(((double) (color)/QuantumRange),1.0/(double) blue_gamma)* \
  QuantumRange)+0.5)))
...
#define XGreenGamma(color) ((Quantum) (green_gamma == 1.0 ? (double) (color) : \
  ((pow(((double) (color)/QuantumRange),1.0/(double) green_gamma)* \
  QuantumRange)+0.5)))
#define XRedGamma(color) ((Quantum) (red_gamma == 1.0 ? (double) (color) : \
  ((pow(((double) (color)/QuantumRange),1.0/(double) red_gamma)* \
  QuantumRange)+0.5)))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  IsTrue() returns MagickTrue if the reason is "true", "on", "yes" or "1".
%
%  The format of the IsTrue method is:
%
%      MagickBooleanType IsTrue(const char *reason)
%
%  A description of each parameter follows:
%
%    o option: either MagickTrue or MagickFalse depending on the reason
%      parameter.
%
...
%
*/
MagickExport MagickBooleanType IsTrue(const char *reason)
{
  if (reason == (char *) NULL)
    return(MagickFalse);
  ...
  XColor *colors,unsigned int number_colors,XColor *color)
{
  ...
  */
  ...
  if (status != False)
    return;
  query_server=(MagickBooleanType) (colors == (XColor *) NULL);
      ...
      if (LocaleCompare(map_type,"list") != 0)
      ...
      else
        {
          ...
          */
          ...
          resource_info->color_recovery=(MagickBooleanType) (i == 0);
        }
      if (status == False)
        {
...
MagickExport void XDestroyResourceInfo(XResourceInfo *resource_info)
{
  ...
  if (resource_info->quantize_info != (QuantizeInfo *) NULL)
    resource_info->quantize_info=DestroyQuantizeInfo(resource_info->quantize_info);
  ...
  const XPixelInfo *pixel,XDrawInfo *draw_info,Image *image)
{
  ...
  */
  ...
  draw_image->storage_class=DirectClass;
  ...
  const XResourceInfo *resource_info,Image *image,XPixelInfo *pixel)
{
  ...
  */
  pixel->highlight_color.red=(unsigned short) (((MagickRealType)
     ...
     (ScaleQuantumToShort((MagickRealType) QuantumRange-HighlightModulate)));
  ...
  XResourceInfo *resource_info)
{
  ...
  */
  ...
  resource_value=XGetResourceClass(database,client_name,"delay",
    "1");
  ...
  resource_value=XGetResourceClass(database,client_name,"immutable",
    LocaleCompare(client_name,"PerlMagick") == 0 ? "True" :
    "False");
  ...
  resource_info->text_font=XGetResourceClass(database,client_name,(char *)
    "font","-*-fixed-medium-r-normal-*-12-*-*-*-*-*-iso8859-1");
  ...
  XResourceInfo *resource_info,XWindowInfo *window)
{
  ...
  */
  ...
  window->storage_class=(unsigned int) visual_info->class;
  ...
  XResourceInfo *resource_info)
{
  ...
  */
  ...
  if ((windows->class_hints == (XClassHint *) NULL) ||
      (windows->manager_hints == (XWMHints *) NULL))
    ThrowMagickFatalException(ResourceLimitError,"UnableToAllocateXHints",
      strerror(errno));
  /*
      ...
      if ((window->immutable == MagickFalse) &&
          ...
          (window->pixel_info->colors == 0))
        {
          ...
          static const unsigned char
            CheckerboardImage[]=
            {
              0x47, 0x49, 0x46, 0x38, 0x37, 0x61, 0x1E, 0x00, 0x1E, 0x00, 0xF0,
              0x01, 0x00, 0x66, 0x66, 0x66, 0x99, 0x99, 0x99, 0x21, 0xF9, 0x04,
              0x00, 0x00, 0x00, 0x00, 0x00, 0x2C, 0x00, 0x00, 0x00, 0x00, 0x1E,
              0x00, 0x1E, 0x00, 0x00, 0x02, 0x42, 0x8C, 0x8F, 0x09, 0xCB, 0xED,
              0x09, 0x91, 0x9B, 0x2D, 0x5A, 0x8A, 0x6D, 0xC4, 0x54, 0x43, 0x3E,
              0x79, 0x0A, 0x58, 0x89, 0x07, 0x59, 0x9A, 0x01, 0xCA, 0xA8, 0x06,
              0xBB, 0xB8, 0x2B, 0x2C, 0xC3, 0x40, 0x4D, 0xBB, 0x36, 0xCE, 0xF2,
              0x36, 0xEA, 0x23, 0x05, 0x41, 0x43, 0x4E, 0x31, 0xA3, 0xCB, 0xA9,
              0x76, 0xC9, 0x5E, 0x13, 0xF8, 0x14, 0x46, 0x89, 0x53, 0x63, 0x15,
              0xB9, 0x54, 0x9A, 0x98, 0xAA, 0x02, 0x00, 0x3B
            };

          pattern=BlobToImage(resource_info->image_info,CheckerboardImage,
            sizeof(CheckerboardImage),&window->image->exception);
          if (pattern != (Image *) NULL)
            {
              ...
              /*
                Composite transparent image on checkerboard.
              */
              checkerboard=CloneImage(window->image,window->image->columns,
                window->image->rows,MagickTrue,&window->image->exception);
              if (checkerboard != (Image *) NULL)
                {
                  (void) TextureImage(checkerboard,pattern);
                  (void) CompositeImage(checkerboard,AtopCompositeOp,
                    window->image,0,0);
                  if (window->image != image)
                    window->image=DestroyImage(window->image);
                  window->image=checkerboard;
                  window->destroy=MagickTrue;
                }
  ...
  const XWindowInfo *window,Image *image,XImage *ximage,XImage *matte_image)
{
  ...
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"%s",image->filename);
  scanline_pad=(MagickBooleanType) (ximage->bytes_per_line-
  ...
  if (ximage->format == XYBitmap)
    {
      ...
      */
      background=(unsigned char)
        (PixelIntensity(&window->pixel_info->foreground_color) <
         PixelIntensity(&window->pixel_info->background_color) ?  0x80 : 0x00);
      foreground=(unsigned char)
        (PixelIntensity(&window->pixel_info->background_color) <
        ...
        case LSBFirst:
        {
          ...
          */
          ...
          if (ximage->format == XYBitmap)
            {
              background=(unsigned char)
                (PixelIntensity(&pixel_info->foreground_color) <
                 PixelIntensity(&pixel_info->background_color) ?  0x80 : 0x00);
              foreground=(unsigned char)
                (PixelIntensity(&pixel_info->background_color) <
            ...
            if (q != (PixelPacket *) NULL)
              {
                for (i=0; i < (long) number_colors; i++)
                {
                  q->red=0;
                  if (map_info->red_max != 0)
                    q->red=ScaleAnyToQuantum(i/map_info->red_mult,
                      map_info->red_max);
                  q->green=0;
                  if (map_info->green_max != 0)
                    q->green=ScaleAnyToQuantum((i/map_info->green_mult) %
                      (map_info->green_max+1),map_info->green_max);
                  q->blue=0;
                  if (map_info->blue_max != 0)
                    q->blue=ScaleAnyToQuantum(i % map_info->green_mult,
                      map_info->blue_max);
                  q->opacity=TransparentOpacity;
                  q++;
                }
...
MagickExport MagickBooleanType XMagickProgressMonitor(const char *tag,
  const MagickOffsetType quantum,const MagickSizeType span,void *client_data)
{
  XWindows
    *windows;
  ...
  const char *window,const char *filename)
{
  ...
  if (display == (Display *) NULL)
    {
      ThrowXWindowException(XServerError,"UnableToOpenXServer",strerror(errno));
      return(MagickFalse);
    }
...
#define MinimumCropArea  (unsigned int) 9
  ...
  */
  ...
  if (status != GrabSuccess)
    {
      ThrowXWindowException(XServerError,"UnableToGrabMouse",strerror(errno));
      return((Window) NULL);
    }
  /*
    Select a window.
