      ...
      for (x=0; x < (long) image->columns; x+=count)
      {
        ...
        */
        for (count=1; (x+count) < (long) image->columns; count++)
          if (ColorMatch(q,q+count) == MagickFalse)
            break;
  ...
  */
  for (y=0; (y < (long) image->rows) && (cube_info->colors < 256); y++)
  {
    ...
    for (x=0; x < (long) image->columns; x+=count)
    {
      ...
      */
      for (count=1; (x+count) < (long) image->columns; count++)
        if (ColorMatch(p,p+count) == MagickFalse)
          break;
...
MagickExport QuantizeInfo *CloneQuantizeInfo(const QuantizeInfo *quantize_info)
{
  ...
  if (clone_info == (QuantizeInfo *) NULL)
    ThrowMagickFatalException(ResourceLimitFatalError,
      "UnableToAllocateQuantizeInfo",strerror(errno));
...
%
%      MagickBooleanType Dither(CubeInfo *cube_info,Image *image,
...
%
*/
static MagickBooleanType Dither(CubeInfo *cube_info,Image *image,
  const unsigned int direction)
{
  ...
  unsigned long depth)
{
  ...
  */
  cube_info->cache=(long *)
    AcquireMagickMemory((size_t) (1L << 18)*sizeof(*cube_info->cache));
...
MagickExport MagickBooleanType GetImageQuantizeError(Image *image)
{
  ...
  image->error.normalized_mean_error=(double) (mean_error/area/QuantumRange/QuantumRange);
  ...
  const MagickBooleanType dither)
{
  ...
  */
  ...
  quantize_info.colorspace=image->matte != MagickFalse ? TransparentColorspace :
    RGBColorspace;
  ...
  if (status != MagickFalse)
    {
      ...
      */
      ...
      for ( ; image != (Image *) NULL; image=GetNextImageInList(image))
      {
        quantize_info.colorspace=image->matte != MagickFalse ?
          TransparentColorspace : RGBColorspace;
        status=AssignImageColors(cube_info,image);
        if (status == MagickFalse)
          break;
      }
  ...
  const unsigned long levels,const MagickBooleanType dither)
{
  ...
  */
  ...
  if (posterize_image == (Image *) NULL)
    return(MagickFalse);
  if (AllocateImageColormap(posterize_image,levels*levels*levels) == MagickFalse)
    {
  ...
  l--;
  posterize_image->columns=(unsigned long) (l*l*l);
  posterize_image->rows=1;
  q=SetImagePixels(posterize_image,0,0,posterize_image->columns,1);
  if (q == (PixelPacket *) NULL)
    {
      posterize_image=DestroyImage(posterize_image);
      return(MagickFalse);
    }
  indexes=GetIndexes(posterize_image);
  n=0;
  for (i=0; i < l; i++)
    for (j=0; j < l; j++)
      for (k=0; k < l; k++)
  ...
  if (SyncImagePixels(posterize_image) == MagickFalse)
    {
      posterize_image=DestroyImage(posterize_image);
      return(MagickFalse);
    }
  ...
  Image *image)
{
  ...
  if ((image->storage_class == PseudoClass) && (image->colors <= number_colors))
    return(MagickTrue);
