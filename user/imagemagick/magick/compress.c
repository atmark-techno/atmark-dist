  ...
  Image *image)
{
  ...
  *jpeg_info->filename='\0';
  (void) strcpy(jpeg_info->magick,"JPEG");
  length=0;
  ...
  Image *image)
{
  ...
  *jp2_info->filename='\0';
  (void) strcpy(jp2_info->magick,"JP2");
  length=0;
  ...
  const unsigned long quality,unsigned char *pixels)
{
  ...
  return((MagickBooleanType) (status == Z_OK));
}
#else
MagickExport MagickBooleanType ZLIBEncodeImage(Image *image,
  const size_t magick_unused(length),const unsigned long magick_unused(quality),
