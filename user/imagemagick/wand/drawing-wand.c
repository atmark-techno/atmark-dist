...
#define WandColorMatch(p,q) ((MagickBooleanType) (((p)->red == (q)->red) && \
...
*/
static int
  MvgPrintf(DrawingWand *,const char *,...)
    magick_attribute((format (printf,2,3))),
  MvgAutoWrapPrintf(DrawingWand *,const char *,...)
    magick_attribute((format (printf,2,3)));
...
static int MvgPrintf(DrawingWand *wand,const char *format,...)
{
  ...
  if (wand->mvg_alloc < (wand->mvg_length+10*MaxTextExtent))
    {
      size_t realloc_size=wand->mvg_alloc+alloc_size;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  ClearDrawingWand() clear resources associated withe the drawing wand.
%
  ...
  const double y,const PaintMethod paint_method)
{
  ...
  (void) MvgPrintf(wand, "color %g,%g '%s'\n",x,y,
    MagickOptionToMnemonic(MagickMethodOptions,(long) paint_method));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
{
  return(1.0-DrawGetFillAlpha(wand));
}
...
WandExport double DrawGetFillAlpha(const DrawingWand *wand)
{
  ...
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  return(((double) (QuantumRange-CurrentContext->fill.opacity))/QuantumRange);
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
{
  return(1.0-DrawGetStrokeAlpha(wand));
}
...
WandExport double DrawGetStrokeAlpha(const DrawingWand *wand)
{
  ...
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  return (1.0-(((double) CurrentContext->stroke.opacity)/QuantumRange));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
*/
WandExport char *DrawGetVectorGraphics(const DrawingWand *wand)
{
  char
    element[MaxTextExtent],
    *xml;
  ...
  (void) ConcatenateString(&xml,"<drawing-wand>\n");
  (void) FormatMagickString(element,MaxTextExtent,
    "  <pointsize>%g</pointsize>\n",CurrentContext->pointsize);
  (void) ConcatenateString(&xml,element);
  (void) ConcatenateString(&xml,"  <vector-graphics>\n");
  if (wand->mvg != (char *) NULL)
    (void) ConcatenateString(&xml,wand->mvg);
  (void) ConcatenateString(&xml,"  </vector-graphics>\n");
  (void) ConcatenateString(&xml,"</drawing-wand>\n");
  return(xml);
}
...
WandExport void DrawPathClose(DrawingWand *wand)
{
  ...
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  (void) MvgAutoWrapPrintf(wand,"%s",
    wand->path_mode == AbsolutePathMode ? "Z" : "z");
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  const double x,const double y)
{
  ...
  if ((wand->path_operation != PathCurveToOperation) ||
    ...
    (void) MvgAutoWrapPrintf(wand," %g,%g %g,%g %g,%g",
      x1,y1,x2,y2,x,y);
}
  ...
  const PathMode mode,const double x,const double y)
{
  ...
  if ((wand->path_operation != PathCurveToQuadraticBezierSmoothOperation) ||
      (wand->path_mode != mode))
    {
      ...
      wand->path_mode=mode;
      (void) MvgAutoWrapPrintf(wand, "%c%g,%g",
  ...
  const double x,const double y)
{
  ...
  if ((wand->path_operation != PathEllipticArcOperation) ||
    ...
    (void) MvgAutoWrapPrintf(wand," %g,%g %g %u %u %g,%g",
  ...
  const PathMode mode,const double x)
{
  ...
  if ((wand->path_operation != PathLineToHorizontalOperation) ||
      (wand->path_mode != mode))
    {
      ...
      wand->path_mode=mode;
      (void) MvgAutoWrapPrintf(wand,"%c%g",
  ...
  const unsigned long number_coordinates,const PointInfo *coordinates)
{
  ...
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  MvgAppendPointsCommand(wand,"polyline",number_coordinates,
    coordinates);
}

/*
  ...
  const char *clip_path)
{
  ...
  if ((CurrentContext->clip_path == NULL) ||
      (wand->filter_off != MagickFalse) ||
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->fill_rule != fill_rule))
    {
      ...
      (void) MvgPrintf(wand, "clip-rule '%s'\n",
        MagickOptionToMnemonic(MagickFillRuleOptions,(long) fill_rule));
    }
}

/*
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->clip_units != clip_units))
    {
      ...
      (void) MvgPrintf(wand, "clip-units '%s'\n",
        MagickOptionToMnemonic(MagickClipPathOptions,(long) clip_units));
    }
}

/*
  ...
  const PixelWand *fill_wand)
{
  ...
  if ((wand->filter_off != MagickFalse) ||
      (WandColorMatch(current_fill,&new_fill) == MagickFalse))
    {
...
{
  DrawSetFillAlpha(wand,1.0-fill_opacity);
}
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->fill_rule != fill_rule))
    {
      ...
      (void) MvgPrintf(wand, "fill-rule '%s'\n",
        MagickOptionToMnemonic(MagickStretchOptions,(long) fill_rule));
    }
}

/*
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->stretch != font_stretch))
    {
      ...
      (void) MvgPrintf(wand, "font-stretch '%s'\n",
        MagickOptionToMnemonic(MagickStretchOptions,(long) font_stretch));
    }
}

/*
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->style != style))
    {
      ...
      (void) MvgPrintf(wand, "font-style '%s'\n",
        MagickOptionToMnemonic(MagickStyleOptions,(long) style));
    }
}

/*
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->gravity != gravity))
    {
      ...
      (void) MvgPrintf(wand,"gravity '%s'\n",
        MagickOptionToMnemonic(MagickGravityOptions,(long) gravity));
    }
}

/*
  ...
  const PixelWand *stroke_wand)
{
  ...
  if ((wand->filter_off != MagickFalse) ||
      (WandColorMatch(current_stroke,&new_stroke) == MagickFalse))
    {
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->linecap != linecap))
    {
      ...
      (void) MvgPrintf(wand,"stroke-linecap '%s'\n",
        MagickOptionToMnemonic(MagickLineCapOptions,(long) linecap));
    }
}

/*
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->linejoin != linejoin))
    {
      ...
      (void) MvgPrintf(wand, "stroke-linejoin '%s'\n",
        MagickOptionToMnemonic(MagickLineJoinOptions,(long) linejoin));
    }
}

/*
...
WandExport void DrawSetStrokeOpacity(DrawingWand *wand,
  const double stroke_opacity)
{
  DrawSetStrokeAlpha(wand,1.0-stroke_opacity);
}
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->align != alignment))
    {
      ...
      (void) MvgPrintf(wand,"text-align '%s'\n",
        MagickOptionToMnemonic(MagickAlignOptions,(long) alignment));
    }
}

/*
  ...
  if ((wand->filter_off != MagickFalse) ||
      (CurrentContext->decorate != decoration))
    {
      ...
      (void) MvgPrintf(wand,"decorate '%s'\n",
        MagickOptionToMnemonic(MagickDecorationOptions,(long) decoration));
    }
}

/*
  ...
  const PixelWand *under_wand)
{
  ...
  if ((wand->filter_off != MagickFalse) ||
      (WandColorMatch(&CurrentContext->undercolor,&under_color) == MagickFalse))
    {
  ...
  const char *xml)
{
  ...
  for (q=(char *) xml; *q != '\0'; )
  {
    ...
    */
    ...
    if (LocaleCompare(keyword,"<drawing-wand") == 0)
      continue;
    ...
    switch (*keyword)
    {
      ...
      case 'p':
      {
        if (LocaleCompare((char *) keyword,"pointsize") == 0)
          {
            CurrentContext->pointsize=atof(token);
            break;
          }
        break;
      }
      ...
      case 'v':
      {
        if (LocaleCompare((char *) keyword,"vector-graphics") == 0)
          {
            register char
              *p;
            ...
            while (*q != '\0')
            {
              if (LocaleCompare(keyword,"<vector-graphics") == 0)
                break;
              GetMagickToken(q,&q,token);
            }
            wand->mvg=(char *) AcquireMagickMemory((q-p+1)*sizeof(*wand->mvg));
            (void) strncpy(wand->mvg,p,(size_t) (q-p));
            wand->mvg[q-p]='\0';
