...
\title{Snort\texttrademark  Users Manual\\  2.6.0 }
...
and the associated Stream4 reassemble plugin. The \texttt{stream4\_reassemble}
...
\hline
\texttt{cache\_clean\_percent} & Purges this percent of least-recently used sessions from the session cache (overrides \texttt{cache\_clean\_sessions}).\\
\hline
\texttt{cache\_clean\_sessions} & Purges this number of least-recently used sessions from the session cache.\\
\hline
...
\hline
\texttt{max\_sessions} & Sets the maximum number of simultaneous sessions.\\
\hline
\texttt{noinspect} & Disables stateful inspection.\\
\hline
\texttt{ttl\_limit} & Sets the delta value that will set off an evasion alert.\\
\hline
\texttt{self\_preservation\_threshold} & Set limit on number of sessions before entering self-preservation mode (only reassemble data on the default ports).\\
\hline
\texttt{self\_preservation\_period} & Set length of time (seconds) to remain in self-preservation mode.\\
\hline
\texttt{suspend\_threshold} & Sets limit on number of sessions before entering suspend mode (no reassembly).\\
\hline
\texttt{suspend\_period} & Sets length of time (seconds) to remain in suspend mode.\\
\hline
\texttt{server\_inspect\_limit} & Restricts inspection of server traffic to this many bytes until another client request is seen (ie: client packet with data).\\
\hline
...
\begin{note}
Some configuration options have an argument of \texttt{yes} or \texttt{no}.  This
