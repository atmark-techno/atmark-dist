...
void SMTP_ParseArgs(u_char *args)
{
    ...
    _smtp_config.ports[SMTP_DEFAULT_SERVER_PORT] = 1;
    _smtp_config.ports[XLINK2STATE_DEFAULT_PORT] = 1;
    ...
    while ( smtp_cmd->name != NULL )
    {
        ...
        AddAlertCmd(smtp_cmd->name, smtp_cmd->id, 0);
        smtp_cmd++;
    }
...
static int ProcessPorts(char *ErrorString, int ErrStrLen)
{
    ...
    _smtp_config.ports[SMTP_DEFAULT_SERVER_PORT] = 0;
    _smtp_config.ports[XLINK2STATE_DEFAULT_PORT] = 0;
