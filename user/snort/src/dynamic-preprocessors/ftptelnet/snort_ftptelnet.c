 ...
 *                         int iGenerator, int *alert_flags)
 *
 ...
 *            alert_flags   => pointer to indicate what alerts we set
 *
 * Returns: int     => an error code integer (0 = success,
 *                     >0 = non-fatal error, <0 = fatal error)
 *
                                ...
                                int iGenerator, int *alert_flags)
{
     ...
     * don't log it again.
     */
    if(alert_flags && (*alert_flags & uiMask))
    {
        return FTPP_SUCCESS;
    }
    ...
    /*
     * Set the alert_flags bit so we don't log the event on a reassembled
     * stream.
     */
    if(alert_flags)
        *alert_flags |= uiMask;
...
static inline int LogFTPEvents(FTP_SESSION *FtpSession)
{
    ...
    return LogFTPPEvents(&GenEvents, iGenerator,
                         &FtpSession->ftp_alert_flags);
}
...
static inline int LogTelnetEvents(TELNET_SESSION *TelnetSession)
{
    ...
    return LogFTPPEvents(&GenEvents, iGenerator,
                         &TelnetSession->telnet_alert_flags);
}
