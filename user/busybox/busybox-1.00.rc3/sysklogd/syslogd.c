...
static int logFileSize = 16 * 1024;
...
struct RemoteLogInfo {
    ...
    int filterlevel;        /* (7 - LOG_xxx) only logs messages at priority 
                             * LOG_xxx and higher.
...
static const char *msys_tgt, *msys_smtpd, *msys_sender;
static FILE *msys_p;
...
static void logOneMessage(int pri, const char *msg, const char *timestamp, struct timeval *tv)
{
	...
	if (remote_logging) {
		char buffer[1024];
		int i;
		snprintf(buffer, 1024, "<%d>%s %s %s %s%s", pri, timestamp,
			LocalHostName, msg, iso_time, content);
		...
		writev_retry:
			if (-1 == sendto(remote_log_info[i].fd, buffer, strlen(buffer), 0,
			   (struct sockaddr *)&remote_log_info[i].saremote,
			   sizeof(remote_log_info[i].saremote))) {
				time_t now;
...
#endif
#ifdef CONFIG_FEATURE_REMOTE_LOG
				if (!remote_logging && strcmp(p, "syslog") == 0) {
...
#ifdef CONFIG_FEATURE_REMOTE_LOG
		case 'R':
			remote_log_info[0].hostname = bb_xstrdup(optarg);
