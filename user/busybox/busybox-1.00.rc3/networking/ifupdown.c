...
#ifdef CONFIG_FEATURE_IFUPDOWN_FLATFSD
#define INTERFACES_CONFIG "/etc/config/interfaces"
#else
#define INTERFACES_CONFIG "/etc/network/interfaces"
#endif
...
static void set_environ(struct interface_defn_t *iface, char *mode)
{
	...
	my_environ = xmalloc(sizeof(char *) * (n_env_entries + 1 /* for final NULL */ ));
...
extern int ifupdown_main(int argc, char **argv)
{
	...
	const char *interfaces = INTERFACES_CONFIG;
