...
void reload_inittab()
{
	...
	/* do the multidimensional malloc with only one more malloc instead of many */
	saveline[0] = (char *)malloc(inittab_size * CMDSIZ * sizeof(char));
	if (!saveline[0]) {
		/* error condition */
		err("malloc failed");
		_exit(1);
	}
	for(i = 1; i < inittab_size; i++) {
		saveline[i] = saveline[0] + i * CMDSIZ;
	}
