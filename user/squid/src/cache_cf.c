...
/*
 * $Id: cache_cf.c,v 1.38.6.11.2.10 2004/04/30 23:33:58 dwsquid Exp $
 *
...
parse_icap_service_type(IcapConfig * cfg)
{
    ...
    /* check bypass */
    if ((A->bypass != 0) && (A->bypass != 1)) {
	debug(3, 0) ("invalid bypass value %d\n", A->bypass);
	self_destruct();
    }
...
icap_service_process(icap_service * s)
{
    ...
    uri_parser = urlParse(METHOD_NONE, s->uri);
    if (NULL == uri_parser) {
	debug(3, 0)("Failed to parse ICAP URI on line %d\n", config_lineno);
	self_destruct();
    }
    if (PROTO_ICAP != uri_parser->protocol) {
	debug(3, 0)("ICAP service URI on line %d must start with icap://\n", config_lineno);
	self_destruct();
    }
    s->hostname = xstrdup(uri_parser->host);
    s->port = uri_parser->port;
    s->resource = xstrdup(strBuf(uri_parser->urlpath));
    requestDestroy(uri_parser);
    return 1;
}
